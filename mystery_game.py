...
"""
"""
Student 1: Tanish Parmeet Biswas 
Spire Id: 35228394 

Student 2: Quang Thong Nguyen 
Spire Id: 35008677
"""
import random
...
def build_game():
    ...
    ## Using 1D Array List to list all the available suspects, locations, and items. 
    ...
    ## shuffling the suspects, locations, and items arrays 

    # Per spec: player gets 4 attempts total.
    # main.py copies this into its variable "attempts".
    # max_attempts = 4
    
    max_attempts = 4 ## the maximum attempts here is 4. 
    ...
    clue_tokens = random.randint(1, 2) ## keeping clue_tokens from 1 to 2 
    return suspects,locations,items,max_attempts,clue_tokens ## returns suspects, locations, max attempts, and clue tokens
...
def display_game_state(suspects, locations, items, pos_suspect, pos_location, pos_item, attempts, tokens):
    ...
    print('ECE 122: Mystery Deduction Game') ## Prints ECE 122: Mystery Deduction Game 
    print('DEDUCTION NOTEBOOK') ## prints Deduction Notebook
    print(f'Remaining Attempts: {attempts}') ## prints the parameter value of attempts 
    print(f'Clue Tokens: {tokens}') ## prints the parameter value for available tokens 
    print('\n')
    print("SUSPECTS: \n")
    for i in range (len(pos_suspect)):
        if pos_suspect[i] == True: ## prints all the items in the suspects list which are still marked as possible(True for the correspending index) in the pos_suspect list with i(index) + 1 at front 
            print(f"{str(i+1)} {suspects[i]}")
    print('\n')    
    print("LOCATIONS: \n")
    for j in range (len(pos_location)):
        if pos_location[j] == True: 
            print(f"{str(j+1)} {locations[j]}") ## prints all the items in the locations list which are still marked as possible(True for the corresponding index) in the pos_location list with j(index) + 1 at front
    print('\n')
    print("ITEMS: \n")
    for k in range(len(pos_item)): 
        if pos_item[k] == True: 
            print(f"{str(k+1)} {items[k]}") ## prints all the items in the items list which are still marked as possible(True for the corresponding index) in the pos_item with k(index) + 1 at front 
...
def evaluate_guess(guess_idx, solution_idx):
    ...
    result_list=[] #create list for result
    for i in range(len(guess_idx)): #Run through the list of the input of the player. Each component of the list guess_idx is a number that represents the index of a certain suspect/location/item in their own lists
        if guess_idx[i] == solution_idx[i]: #For each component of the guess_idx list, compare it with the component from the soution_idx list of the same index.
            result_list.append(True) #If the players answer is the same as the solution, add True to the list to indicate it
        else:
            result_list.append(False) #if the answer is not he same as the solution, add False to the list to indicate it
    return result_list # return the result list with just true and false whose value align with suspects, location and item
...
def is_win(result_list):
    ...
    """
    if False in result_list: #The player only win if all of their results are true.
        return False  #Thus, if there appears to be at least 1 False in the result list, return False.
    else:
        return True #Else means that there are no False in the result, so all are True and the player win; therefore return True.
...
def update_all_notebooks(pos_suspect, pos_location, pos_item, guess_idx, result_list):
    ...
    if result_list[0]==False: # result_list[0] is the result for the players guess of suspect.
        pos_suspect[guess_idx[0]] = False
        #guess_idx[0] is the index of the suspect that the player type in
        #When the result is false, we change the list pos_suspect of the index guess_idx[0] to False to indicate the wrong suspect
        #If the result is true, don't change anything because the list start with all True
    if result_list[1]==False:# result_list[1] is the result for the players guess of location.
        pos_location[guess_idx[1]] = False
        #guess_idx[1] is the index of the location that the player type in
        #When the result is false, we change the list pos_location of the index guess_idx[1] to False to indicate the wrong location
        #If the result is true, don't change anything because the list start with all True
    if result_list[2]==False:# result_list[2] is the result for the players guess of item.
        pos_item[guess_idx[2]] = False
        #guess_idx[2] is the index of the item that the player type in
        #When the result is false, we change the list pos_item of the index guess_idx[2] to False to indicate the wrong item
        #If the result is true, don't change anything because the list start with all True
...
def check_and_record_guess(guess_history, guess_idx):
    ...
    penalty = 1 #default penalty is 1
    if guess_idx in guess_history:
        penalty = 2 # if the input the player put in is already in the guess history, change penalty to 2 and return it
        return penalty
    else: guess_history += [guess_idx] #If the input the player put in is not in the guess history, add that input to the guess history and return 1 as penalty 
    return penalty
...
def use_clue_token(possible_list, solution_index):
    ...
    is_eliminated = False ## Variable which returns if elimination occurs or not. Initialising to False 
    for i in range(len(possible_list)): ## iterating through the list from low index to high index 0 to len(possible_list)
        if i != solution_index and possible_list[i] != False: ## checking if the index is NOT equal to the solution_index(correct index) as well as if the current
                                                              ##  iteration or index is False or not, if it is False, then it continues to the next index to check if its True and not the solution, if so
                                                              ## then it changes it to False. 
                                                                  
            possible_list[i]=False ## Value changed to False for the lowest possible value in the iteration
            is_eliminated = True ## return value changed to True to indicate elimination took place 
            break 
    return is_eliminated  ## returns the variable which determines if any value has been eliminated or not 
...
def best_clue_category(pos_suspect, pos_location, pos_item):
    ...
    sus_count = 0  ## Suspect Count 
    loc_count = 0 ## Location Count 
    items_count = 0 ## Items Count 
    for i in range(len(pos_suspect)): 
        if pos_suspect[i] == True: 
            sus_count = sus_count + 1 ## increments the suspect count by 1 if the value is True for that specific index
    for j in range(len(pos_location)): 
        if pos_location[j] == True: 
            loc_count = loc_count + 1 ## increments the location count by 1 if the value is True for that specific index 
    for k in range(len(pos_item)): 
        if pos_item[k] == True: 
            items_count = items_count + 1 ## increments the items count by 1 if the value is True for that specific index 
    if sus_count > loc_count and sus_count > items_count: ## returns 0 if suspect count is the biggest 
        return 0 
    elif loc_count > sus_count and loc_count > items_count: ## returns 1 if location count is the biggest 
        return 1 
    elif items_count > sus_count and items_count > loc_count: ## returns 2 if items count is the biggest 
        return 2 
    elif sus_count == loc_count or sus_count == items_count: ## returns 0 if sus_count = loc_count given items_count is smaller than both or if sus_count is 
                                                             ## equal to items_count given loc_count is smaller, or if all of them are equal. 
        return 0 
    else:                  ## Here it is for if loc_count = items_count and it returns 1 

        return 1 
    ## priority : Suspect Count -> Location Count -> Items Count     
